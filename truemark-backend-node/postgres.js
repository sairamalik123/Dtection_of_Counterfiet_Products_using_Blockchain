...
//     return res
//       .status(400)
//       .json({ valid: false, message: "Missing serial number or one-time key" });
//   }
...
//         .json({
//           valid: false,
//           message: "Invalid serial number or one-time key",
//         });
//     }
...
//       return res
//         .status(400)
//         .json({ valid: false, message: "One-time key has already been used" });
//     }
...
//     "SELECT encrypted_private_key FROM product WHERE serialnumber = $1",
//     [serialNumber]
//   );
...
//       ...profile,
//       image: profile.image
//         ? `http://localhost:5000/file/profile/${profile.image}`
//         : null,
//     }));

//     res.header("Access-Control-Allow-Credentials", true);
//     res.send(modifiedProfiles);
//     console.log("Modified profile data with image URLs sent");
//   } catch (err) {
...
//   console.log("Profile sent successfully for", username);
// });
...
app.post("/addproduct", async (req, res) => {
  ...
  try {
    ...
    // Fetch one_time_key, signature, and public_key after insert
    const result = await client.query(
      `SELECT one_time_key, signature, public_key FROM product WHERE serialnumber = $1`,
      [serialNumber]
    );

    const { one_time_key, signature, public_key } = result.rows[0];
