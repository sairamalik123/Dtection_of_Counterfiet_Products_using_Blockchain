...
//     const response = await axios.get("https://api.apiip.net/ip?apiKey=0d1b4ae003c2c875abbcf679a798b9fdfc4a4e1f");
...
//     setManuLocation("Location not available");
//   }
// };
...
//     console.log("Manufacturer Name:", manuName);
//   };
...
//     const { serialnumber, signature, publicKey, one_time_key } = response.data;

//     // ✅ Set all values returned by backend
//     setSignature(signature);
//     setPublicKey(publicKey);
//     setOneTimeKey(one_time_key); // ✅ correct key from DB
...
//     />
//     <Button
//       onClick={() => navigator.clipboard.writeText(oneTimeKey)}
//       sx={{
//         position: 'absolute',
//         right: 0,
//         top: 0,
...
const AddProduct = () => {
  ...
  const [currLongtitude, setCurrLongtitude] = useState(null);
    ...
    if (currLatitude && currLongtitude) {
      fetch(`https://apiip.net/api/check?accessKey=2c0f29da-08b1-497a-9f94-15987b07b114`)
        ...
        .catch((err) => {
          ...
          setCurrLocation("Unknown Location");
        });
    }
  ...
  const handleImage = async (e) => {
    const file = e.target.files[0];
    setImage({ file: file, filepreview: URL.createObjectURL(file) });
  };
  ...
  const registerProduct = async (e, dbSerialNumber) => {
    ...
    try {
      ...
      if (userRole !== "Manufacturer") {
        alert("Only a Manufacturer can register products.");
        return;
      }
      ...
      const blockchainSignature = await signer.signMessage(ethers.utils.arrayify(hash));

      const registerTxn = await productContract.registerProduct(
        dbSerialNumber.toString(),
        name,
        brand,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading("Adding product to database...");
      if (image.file) await uploadImage(image);
      const dbSerialNumber = await addProductDB(e);
      setSerialNumber(dbSerialNumber);
  ...
  return (
    <Box sx={{ backgroundImage: `url(${bgImg})`, minHeight: "80vh", backgroundRepeat: "no-repeat", position: "absolute", left: 0, right: 0, top: 0, bottom: 0, backgroundSize: "cover", zIndex: -2, overflowY: "scroll" }}>
      <Paper elevation={3} sx={{ width: "400px", margin: "auto", marginTop: "3%", marginBottom: "10%", padding: "3%", backgroundColor: "#e3eefc" }}>
        ...
        <form onSubmit={handleSubmit}>
          {serialNumber && (
            <TextField fullWidth label="Generated Serial Number" margin="normal" variant="outlined" value={serialNumber} InputProps={{ readOnly: true }} />
          )}

          {oneTimeKey && (
            <Box sx={{ position: 'relative', mt: 2 }}>
              <TextField fullWidth label="One-Time Key" value={oneTimeKey} InputProps={{ readOnly: true }} />
              <Button onClick={() => navigator.clipboard.writeText(oneTimeKey)} sx={{ position: 'absolute', right: 0, top: 0, height: '100%', borderRadius: 0 }}>
                Copy
              </Button>
            </Box>
          )}

          <TextField fullWidth label="Name" margin="normal" variant="outlined" value={name} onChange={(e) => setName(e.target.value)} required />
          <TextField fullWidth label="Brand" margin="normal" variant="outlined" value={brand} onChange={(e) => setBrand(e.target.value)} required />
          ...
          <Button variant="outlined" component="label" fullWidth sx={{ marginTop: "3%", marginBottom: "3%" }}>
            Upload Image
            <input type="file" hidden ref={fileInputRef} onClick={() => (fileInputRef.current.value = null)} onChange={handleImage} />
          </Button>
          ...
          {qrData && (
            <>
              ...
              <Box sx={{ display: "flex", justifyContent: "center", mt: 3 }}>
                <Button variant="outlined" fullWidth onClick={downloadQR}>Download QR Code</Button>
              </Box>
            </>
          )}

          {loading && (
            <Typography variant="body2" sx={{ textAlign: "center", mt: 3 }}>{loading}</Typography>
          )}

          <Button variant="contained" type="submit" fullWidth sx={{ mt: 3, backgroundColor: "#98b5d5", '&:hover': { backgroundColor: "#618dbd" } }} disabled={!name || !brand || loading}>
            Add Product
          </Button>

          <Box sx={{ width: "100%", display: "flex", justifyContent: "center" }}>
            <Button onClick={handleBack} sx={{ mt: 5 }}>Back</Button>
          </Box>
        </form>
      </Paper>
    </Box>
