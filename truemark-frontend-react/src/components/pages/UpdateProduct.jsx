...
//             overflowY: "scroll"
//         }}>
//             <Paper elevation={3} sx={{ width: "450px", margin: "auto", marginTop: "3%", marginBottom: "1%", padding: "3%", backgroundColor: "#e3eefc" }}>
...
const UpdateProduct = () => {
    ...
    //     const historyData = product[5]; // an array of history entries

    //     const hist = historyData.map(entry => {
    //         const actor = entry[1];
    //         const location = entry[2].replace(/;/g, ",");
    //         const timestamp = entry[3];
    //         const sold = entry[4];
    ...
    const setData = (product) => {
        ...
        const hist = historyData.map((entry, index) => {
            const actor = entry[1] || "Unknown Actor"; // Default if actor is undefined
            const location = entry[2] && entry[2] !== "undefined, undefined, undefined"
                ? entry[2].replace(/;/g, ",") // Format location properly
                : "Location not available"; // Default if location is undefined or invalid
            const timestamp = entry[3] || 0; // Default timestamp if undefined
            const sold = entry[4] === "true"; // Ensure correct boolean conversion
