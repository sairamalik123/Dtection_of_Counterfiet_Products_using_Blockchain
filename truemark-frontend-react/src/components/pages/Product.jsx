...
const Product = () => {
      ...
      if (typeof qrData === "string") {
        try {
          ...
          if (parsed.contract && parsed.serialNumber && parsed.signature) {
            ...
            if (
              parsed.contract.toLowerCase() === CONTRACT_ADDRESS.toLowerCase()
            ) {
  ...
  const cleanSerialNumber = (serial) => {
    ...
    if (cleaned.includes("0xff640E131188aAf6E898a53E7969054327c7A5aA")) {
      cleaned = cleaned
        .replace("0xff640E131188aAf6E898a53E7969054327c7A5aA", "")
        .replace(/[,\-_]/g, "");
    }
...
console.log("First history entry:", productHistory[0]);
      ...
      if (
        ...
        //   const actor = entry[0] || "Unknown Actor";
        //   let location = entry[1] || "Unknown Location";
        //   const timestamp = entry[2] ? Number(entry[2]) : 0;
        //   const isSoldFlag = entry[3] === true || entry[3] === "true";

        //   // Clean location (handle malformed or undefined cases)
        //   if (
        //     typeof location === "string" &&
        //     (location.toLowerCase().includes("undefined") ||
        //       location.trim() === "" ||
        //       location === ", ,")
        //   ) {
        ...
        //   // Assume entry is an array: [actor, location, timestamp, isSold]
        //   const actor = entry[0] || "Unknown Actor";
        //   let location = entry[1] || "Unknown Location";
        //   const timestamp = entry[2] ? Number(entry[2]) : 0;
        //   const isSoldFlag = entry[3] === true || entry[3] === "true";

        //   // Clean malformed locations
        //   if (
        //     typeof location === "string" &&
        //     (location.toLowerCase().includes("undefined") ||
        //       location.trim() === "" ||
  ...
  const rawTimestamp = entry.timestamp || entry[2] || 0;
  const isSoldFlag = entry.isSold || entry[3] || false;

  const timestamp = BigNumber.isBigNumber(rawTimestamp) ? rawTimestamp.toNumber() : Number(rawTimestamp);
              ...
              {history.length === 0 ? (
                ...
                history.map((item, index) => (
                  <TimelineItem key={index}>
                    ...
                    <TimelineContent>
                      <Typography>{String(item.actor)}</Typography>
<Typography>{String(item.location)}</Typography>
